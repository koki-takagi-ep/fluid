 ...
 * キャビティ流れ（PISO法）
 *
 * PISO法を使用したキャビティ流れのシミュレーション
 * Projection法、SIMPLE法との比較用
 */
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 4) nCorrectors = std::atoi(argv[4]);

    // レイノルズ数を再計算
    Re = U_lid * L / nu;

    std::cout << "=== Lid-Driven Cavity Flow (PISO Method) ===" << std::endl;
    ...
    // CSV出力（PISO法）
    fluid::CSVWriter writer("output/cavity_piso");
